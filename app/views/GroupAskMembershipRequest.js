    ...
    if (this.props.isAllowedPending) {
      return (
        <View style={{flex: 1, justifyContent: 'center'}}>
          <Text style={{fontSize: 20, textAlign: 'center'}}>{i18next.t('membershipRequest:allowed-pending')}</Text>
        </View>
      );
    }
    return (
      <View style={{backgroundColor: '#f0f0f0'}}>
        <Text style={{fontSize: 20}}>
          {i18next.t('membershipRequest:info')}
        </Text>
        <View style={{backgroundColor: '#FFF', height: 80}}>
          <Input
            placeholder={i18next.t('membershipRequest:placeholder')}
            multiline={true}
        ...
        <Button onPress={this.onSendRequest.bind(this)}
          type='green'
          label={i18next.t('membershipRequest:send')} />
      </View>
    );
  }
  onSendRequest () {
    app.client.groupRequest(this.props.id, this.state.motivations, function (response) {
      if (response.success) {
        ...
        if (response.err === 'allow-pending') {
          return alert.show(i18next.t('membershipRequest:allow-pending'));
        }
        return alert.show(i18next.t('membershipRequest:unknown-error'))
      }
    });
  }
}
